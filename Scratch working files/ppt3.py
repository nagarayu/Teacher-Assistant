...
# Ensure folderPath and savedPptFolder exist and are cleared before saving new slides
...
# Create the "Saved ppt" folder if it doesn't exist
def create_saved_ppt_folder():
...
# Ask user to upload the PowerPoint file and convert it to images
ppt_file = upload_ppt()
if ppt_file:
...
# Save annotated images to "Saved ppt" folder
create_saved_ppt_folder()
for i, file_name in enumerate(sorted(os.listdir(folderPath))):
